...
st.markdown("""
    ...
    .carrello-item { background-color: #262730; padding: 10px; border-radius: 10px; margin-bottom: 5px; border-left: 5px solid #FF4B4B; }
    </style>
...
def suona_notifica():
    # Suono più squillante per l'invio
...
def stampa_scontrino(tavolo, prodotti, totale):
    ...
    html = f"""
    <div id="s" style="font-family:monospace; width:75mm; color:black; background:white; padding:10px; border:1px solid #ccc;">
...
if ruolo == "banco":
    ...
    # AUTO-AGGIORNAMENTO OGNI 3 SECONDI (Solo per la console)
    if "last_refresh" not in st.session_state: st.session_state.last_refresh = time.time()
    if time.time() - st.session_state.last_refresh > 3:
        st.session_state.last_refresh = time.time()
        st.rerun()

    tab1, tab2, tab3 = st.tabs(["📋 ORDINI IN TEMPO REALE", "🥐 STOCK", "⚙️ LISTINO"])
    
    with tab1:
        ordini = carica_ordini()
        nuovi = [o for o in ordini if o['stato'] == "NO"]
        if nuovi:
            st.error(f"⚠️ {len(nuovi)} NUOVI PRODOTTI DA PREPARARE!")
            ...
            for idx, t in enumerate(tavoli):
                with cols[idx % 3]:
                    with st.container(border=True):
                        ...
                        if st.button(f"CHIUDI E STAMPA €{tot:.2f}", key=f"p_{t}", type="primary", use_container_width=True):
                            stampa_scontrino(t, items, tot)
                            salva_ordini([o for o in ordini if str(o['tavolo']) != t])
                            st.rerun()

    # (Tab 2 e 3 rimangono invariate per brevità, ma integrate nel codice finale)
    with tab2:
        st.write("Gestione Stock Brioche...")
        # Codice stock precedente...
    with tab3:
        st.write("Modifica Listino...")
        # Codice listino precedente...
    ...
    if st.session_state.tavolo is None:
        ...
        t_cols = st.columns(4)
        for i in range(1, 21):
            if t_cols[(i-1)%4].button(f"{i}", use_container_width=True):
        ...
        scelta = st.radio("Scegli Categoria:", cats, horizontal=True)
        ...
        for i, (idx, r) in enumerate(prods.iterrows()):
            with p_cols[i % 2]:
                if st.button(f"➕ {r['prodotto']}\n€{r['prezzo']:.2f}", key=f"c_{idx}", use_container_width=True):
                    # Aggiungiamo un ID unico al carrello per poterli eliminare singolarmente
                    item = r.to_dict()
